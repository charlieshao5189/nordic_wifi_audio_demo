...
![recorder build configuraiton](/readme_resources/build_configuraiton_recorder.png)
...
```
Build nRF5340 Audio Broadcast Source (BIS Gateway Mode):

```
west build -b nrf5340_audio_dk/nrf5340/cpuapp --sysbuild --pristine -d build_broadcast_source_tx  -- -DCONFIG_AUDIO_DEV=2 -DCONFIG_TRANSPORT_BIS=y
```

Build nRF5340 Audio Broadcast Sink (BIS Headset Mode):

```
west build -b nrf5340_audio_dk/nrf5340/cpuapp --sysbuild --pristine -d build_broadcast_sink_rx  -- -DCONFIG_AUDIO_DEV=1 -DCONFIG_TRANSPORT_BIS=y
```
